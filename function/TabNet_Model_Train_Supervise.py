...
plt.rcParams['font.sans-serif'] = ['SimHei']    # plt.rcParams设置图形各种属性，这里是字体
plt.rcParams['axes.unicode_minus'] = False      # 图形的字符显示
np.random.seed(10)

data = pd.read_excel('data/bistandard+soft_fill.xlsx')
data_x = data.iloc[:, :-1]    #前面的冒号取行数，后面的冒号取列数 左闭右开区间
...
train_y = np.array(train_y).reshape(-1, 1)    # 转化为1列数组
...
clf = TabNetRegressor(
    verbose=True,
    seed=10,
    optimizer_fn=torch.optim.Adam,   # 优化器Adam
    optimizer_params=dict(lr=2e-2),
    n_a=8,    # 注意力嵌入的宽度
    n_d=8,    # 决策预测层的宽度
    n_steps=3,   # 决策步长
...
# print('1111111111111',pred_y)
pred_y = pred_y.reshape(1, -1)[0]
# print('222222222222222',pred_y)
