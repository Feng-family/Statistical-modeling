...
#                 }
'''预训练，在77、78行加载的本地模型就是这里训练出来的，所以要先运行这部分生成预训练的模型再运行77 78行以后的部分'''
...
# pre_clf.save_model('pretrain/test_pretrain1')

loaded_pretrain = TabNetPretrainer()
loaded_pretrain.load_model('pretrain/test_pretrain1.zip')
clf = TabNetRegressor(optimizer_fn=torch.optim.Adam,     # 优化器选择Adam
                      optimizer_params=dict(lr=2e-2),    #
                      scheduler_params={"step_size": 10,  # how to use learning rate scheduler
                                        "gamma": 0.9},
                      scheduler_fn=torch.optim.lr_scheduler.StepLR,
